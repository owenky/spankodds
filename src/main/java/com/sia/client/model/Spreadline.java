...
public class Spreadline extends Line {
    static final LineIdentity defaultLineIden = new LineIdentity(99999, 99999, 0);
    static Spreadline sl = new Spreadline(defaultLineIden, 99999, 99999, 99999, 99999, 1L);

    private boolean isbestvisitspread = false;
    private boolean isbesthomespread = false;
    private double currentvisitspread;
    private double currentvisitjuice;
    private double currenthomespread;
    private double currenthomejuice;
    private double priorvisitspread;
    private double priorvisitjuice;
    private double priorhomespread;
    private double priorhomejuice;
    private double openervisitspread;
    private double openervisitjuice;
    private double openerhomespread;
    ...
    public Spreadline(LineIdentity lineIdentity, double vs, double vj, double hs, double hj, long ts) {
        this(lineIdentity);
    ...
    public Spreadline(LineIdentity lineIdentity) {
        super(lineIdentity,"spread");
    }


    public Spreadline(LineIdentity lineIdentity, double vs, double vj, double hs, double hj, long ts, double pvs, double pvj, double phs, double phj, long pts) {
        this(lineIdentity);
    ...
    public Spreadline(LineIdentity lineIdentity, double vs, double vj, double hs, double hj, long ts, double pvs, double pvj, double phs, double phj, long pts, double ovs, double ovj, double ohs, double ohj, long ots) {
        this(lineIdentity);
    ...
    public Spreadline(LineIdentity lineIdentity, double vs, double vj, double hs, double hj, long ts, double pvs, double pvj, double phs, double phj, long pts, double ovs, double ovj, double ohs, double ohj, long ots, int mb) {
        this(lineIdentity);
    ...
    public void setBestVisitSpread(boolean b) {
        isbestvisitspread = b;
        if (b) {
            AppController.bestvisitspread.put(getPeriod() + "-" + getGameid(), getBookieObject());
        }
    }
    ...
    public void setBestHomeSpread(boolean b) {
        isbesthomespread = b;
        if (b) {
            AppController.besthomespread.put(getPeriod() + "-" + getGameid(), getBookieObject());
        }
    }
    ...
    public String recordMove(double visitspread, double visitjuice, double homespread, double homejuice, long ts, boolean isopener) {
        ...
        } else if (getGameid() < 10000) // if this is a half move i will throw away
        {
        ...
        BestLines.calculatebestspread(getGameid(), getPeriod());
        BestLines.calculateconsensusspread(getGameid(), getPeriod());
        return sl.whowasbet;
    ...
    private String getShortPrintedSpread(double vspread, double vjuice, double hspread, double hjuice) {
        boolean debug = false;
        if (getPeriod() == 0 && getGameid() == 951 && getBookieid() == 599 && debug) {
            Utils.log(getBookieid() + "...leagueid=" + leagueid + "...SHORTPRINTEDSPREAD=" + vspread + ".." + vjuice + ".vs." + hspread + ".." + hjuice);
        }
        String retvalue = "";
        if (vjuice == 0) {
            return "";
        ...
        if (getPeriod() == 0 && getGameid() == 951 && debug && getBookieid() == 599) {
    ...
    private String getOtherPrintedSpread(double vspread, double vjuice, double hspread, double hjuice) {
        boolean debug = false;
        if (getPeriod() == 0 && getGameid() == 951 && getBookieid() == 599 && debug) {
            Utils.log(getBookieid() + "OTHERPRINTEDSPREAD=" + vspread + ".." + vjuice + ".vs." + hspread + ".." + hjuice);
        }
        String retvalue = "";
        if (vjuice == 0) {
            return "";

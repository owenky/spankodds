...
public void itemStateChanged(ItemEvent e) 
  { 
		...
		sportsbooktreePanel.add(new JScrollPane(sportsbooktree));
					...
					/*if(AppController.LineOpenerAlertNodeList.get(i).leagues!=null)
					{
						int treeRows[]=new int[AppController.LineOpenerAlertNodeList.get(i).leagues.size()];
						for(int i1=0;i1<treeRows.length;i1++)
						{
							treeRows[i1]=(int)AppController.LineOpenerAlertNodeList.get(i).leagues.get(i1);
						}
					...
					if(AppController.LineOpenerAlertNodeList.get(i).bookies!=null)
					{
						int treeRows1[]=new int[AppController.LineOpenerAlertNodeList.get(i).bookies.size()];
						for(int i1=0;i1<treeRows1.length;i1++)
						{
							treeRows1[i1]=(int)AppController.LineOpenerAlertNodeList.get(i).bookies.get(i1);
						}
...
LineAlertOpeners(){
					...
					/*String sportname=(String)sportComboBox.getSelectedItem();
					for(idx=0;idx<AppController.LineOpenerAlertNodeList.size();idx++)
					{
						if(AppController.LineOpenerAlertNodeList.get(idx).SportName.equalsIgnoreCase(sportname))
						{
							break;
						}	
					}*/
...
panel1.setBorder(BorderFactory.createEtchedBorder());
	...
	usecustomsound.addActionListener(new ActionListener() {
 	...
 	testpopup.addActionListener(new ActionListener() {
			 public void actionPerformed(ActionEvent ae)
			{
	...
	set.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae)
			{
				...
				//AppController.LineOpenerAlertNodeList.get(idx).leagues=treeRows;
		...
		//AppController.LineOpenerAlertNodeList.get(idx).bookies=treeRows1;
					//System.out.println("treerows="+treeRows[0]);
					if(treeRows1 != null)
					{
			...
			//AppController.LineOpenerAlertNodeList.get(idx).renotifyvalue=(String)renotifyComboBox.getSelectedItem();
			//AppController.LineOpenerAlertNodeList.get(idx).showpopvalue=(String)popupsecsComboBox.getSelectedItem();
			//AppController.LineOpenerAlertNodeList.get(idx).audiovalue=(String)audioComboBox.getSelectedItem();
...
public static void spreadOpenerAlert(int Gid,int bid,int per,String opener,double newvisitorspread,double newvisitorjuice,double newhomespread,double newhomejuice)
{
	...
	//ArrayList loan=AppController.LineOpenerAlertNodeList;
	...
	if(opener.equals("1"))
	{
		for(int i=0;i<AppController.LineOpenerAlertNodeList.size();i++)
		{
			...
			/*Sport sp=AppController.getSport(sport_id+"");
			...
			if((leagues.contains(lid)||isAllLeaguesSelected)&&(bookeis.contains(bid)||isAllBookiesSelected)&&periods.contains(per)&&isspreadcheck)
			{
				...
				if(isShowpopChecks)
				{
					...
					String popalertname="Alert at:"+hrmin+"Spread Opener:"+getLeagueAbbr(lid)+","+teaminfo;
						...
						//System.out.println(AppController.alertsVector.size());
